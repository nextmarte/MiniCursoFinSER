...
# Imprime o data frame risco_retorno
print(risco_retorno)
...
pesos_ativos <- rbind(pesos_ativos, data.frame(Ativo = "CARTEIRA", Peso = peso_total))
...
# Cria o gráfico
rr <- plotly::ggplotly(ggplot(risco_retorno) +
  aes(x = Risco, y = Retorno, colour = Ativo, size = Peso) + # Adiciona a estética size = Peso
  geom_point() +
  scale_color_hue(direction = 1) +
...
# adicionando pesos ao data frame
